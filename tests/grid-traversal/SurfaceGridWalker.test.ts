/**
 * SurfaceGridWalker Tests
 *
 * Verifies the grid traversal utility:
 * - Grid density controls number of test points
 * - Sphere at density 5 produces 0 stuck points
 * - Stuck detection fires for degenerate positions
 *
 * Requires DOM/WebGL/Three.js mocks for headless vitest execution.
 */
...
// (Copied from all-surfaces-verification.test.ts)
// ---------------------------------------------------------------------------
...
import { SurfaceGridWalker } from './SurfaceGridWalker';

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

describe('SurfaceGridWalker', () => {
  it('grid density parameter controls number of test points', () => {
    // density=5 → 5×5=25 points (sphere has no pole-skipping at density 5 for v in [0.1,0.9])
    const result = SurfaceGridWalker.runGrid('sphere', 5, 10);

    // All points should be generated (sphere has pole avoidance so some may be skipped at edges)
    expect(result.points.length).toBeGreaterThan(0);
    expect(result.points.length).toBeLessThanOrEqual(25);
    expect(result.gridDensity).toBe(5);
    expect(result.surface).toBe('sphere');
  });

  it('density=3 produces fewer points than density=5', () => {
    const small = SurfaceGridWalker.runGrid('sphere', 3, 5);
    const larger = SurfaceGridWalker.runGrid('sphere', 5, 5);

    expect(larger.points.length).toBeGreaterThan(small.points.length);
  });

  it('result has required shape: stuckCount + passCount = points.length', () => {
    const result = SurfaceGridWalker.runGrid('sphere', 3, 10);

    expect(result.stuckCount + result.passCount).toBe(result.points.length);
    expect(result.durationMs).toBeGreaterThanOrEqual(0);
  });

  it('each GridPoint has required fields', () => {
    const result = SurfaceGridWalker.runGrid('sphere', 3, 10);

    for (const point of result.points) {
      expect(typeof point.u).toBe('number');
      expect(typeof point.v).toBe('number');
      expect(point.worldPos).toBeDefined();
      expect(typeof point.stuck).toBe('boolean');
      expect(typeof point.stuckReason).toBe('string');
      expect(typeof point.distanceMoved).toBe('number');
    }
  });

  it('sphere at density 5 produces 0 stuck points', () => {
    // Sphere is the reference surface — should be fully traversable everywhere
    const result = SurfaceGridWalker.runGrid('sphere', 5, 60);

    expect(result.stuckCount).toBe(0);
    expect(result.passCount).toBe(result.points.length);
  }, 60_000); // generous timeout for 25 points × 4 dirs × 60 frames

  it('stuck detection fires when distanceMoved is below threshold', () => {
    // All points on a valid surface should have distanceMoved reported
    const result = SurfaceGridWalker.runGrid('torus', 3, 10);

    for (const point of result.points) {
      if (point.stuck) {
        expect(point.distanceMoved).toBeLessThan(0.05);
        expect(point.stuckReason.length).toBeGreaterThan(0);
      } else {
        expect(point.distanceMoved).toBeGreaterThanOrEqual(0.05);
      }
    }
  });
});
