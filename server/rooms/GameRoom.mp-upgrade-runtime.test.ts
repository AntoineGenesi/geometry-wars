...
describe('GameRoom MP weapon upgrade runtime parity', () => {
  it('does not emit default-unlocked Blaster Focus upgrades for a fresh MP player', () => {
    const room = makeRoom();
    ...
    expect(room.playerActiveUpgradeNodes.get('p1').get(WeaponType.Standard).size).toBe(0);
  });

  it('emits earned Blaster Focus upgrades after they become server-authoritative', () => {
    const room = makeRoom();
    const player = makePlayer();
    room.state.players.set('p1', player);
    room.playerActiveUpgradeNodes.set('p1', new Map([
      [WeaponType.Standard, new Set(STANDARD_BLASTER_EARLY_AUTO_UNLOCK_NODE_IDS)],
    ]));
  ...
  it('clears private and schema upgrade state together at round reset', () => {
    ...
    expect([...activeUpgradeNodes.keys()]).toEqual([]);
  });
