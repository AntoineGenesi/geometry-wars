...
export interface WaveEnemy {
   ...
   * Set by WaveComposer for late-game wave scaling (up to 50 for Snake, 20 for GiantSnake).
   * Omit to use the default caps (14 for Snake, 7 for GiantSnake).
   */
  maxSegments?: number;
}
...
export class EnemySpawner {
  ...
  spawn(type: EnemyType, surfaceU?: number, surfaceV?: number, tier: number = 0, skipSpawnWarning: boolean = false, continuousDifficultyLevel?: number, maxSegments?: number): BaseEnemy {
    ...
    switch (type) {
      ...
      case 'giant_rocket':
        ...
        break;
      case 'giant_snake':
        enemy = new GiantSnake(u, v, maxSegments);
        break;
      case 'giant_neutron':
  ...
  spawnWave(waveEnemies: WaveEnemy[]): void {
    ...
    for (const waveEnemy of modifiedWave) {
      ...
      const maxSegments = waveEnemy.maxSegments;
  ...
  /**
   * Spawn a wave generated by WaveComposer — strategically varied enemy composition.
   *
   * Unlike spawnWave() which takes explicit WaveEnemy lists, spawnComposedWave()
   * derives the wave composition from the wave number and player count:
   * - Cycles through 4 archetypes (Swarm, Support, Chaos, SingleBig)
   * - Breathing-room "Rest" waves every 15 waves (only 4 enemies)
   * - Snakes grow to 50 segments in late game
   * - Enemy count scales with player count (1p=0.8x ... 4p=2.0x)
   *
   * @param waveNumber - Current wave number (1-based)
   * @param playerCount - Number of active players (1-4)
   */
  spawnComposedWave(waveNumber: number, playerCount: number = 1): void {
    const waveEnemies = waveComposer.composeWave(waveNumber, playerCount);
    this.spawnWave(waveEnemies);
  }
