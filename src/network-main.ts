...
async function main() {
  ...
  gameModeRow.style.cssText = 'display:flex;gap:8px;margin-bottom:8px;margin-top:8px;';
  ...
  modeSelectorDiv.appendChild(gameModeRow);

  // Win condition section — shown inline when PvP or PvPvE is selected
  ...
  modeSelectorDiv.appendChild(winConditionSection);
  ...
  // Initialize Co-op as selected by default
  updateLobbyUI();
  ...
  modeSelectorDiv.appendChild(waveTypeLabelEl);
    ...
    if (_netMainDebug) {
      ...
      networkPlayers.forEach((p, id) => {
        ...
        otherPlayers.push({
          ...
          zoneTime: (p as any).zoneTime ?? 0,
          enemyKills: (p as any).enemyKills ?? 0,
